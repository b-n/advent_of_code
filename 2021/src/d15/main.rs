...
fn p01(p: &Path) -> Option<usize> {
    ...
    while next.len() > 0 {
        let mut sorted_points: Vec<(Point, Cost)> = next.iter()
            .map(|(p, cost)| (*p, *cost))
            .collect();
        ...
        //print!("{}[2j{}[{};{}H{}", 27 as char, 27 as char, point.0, point.1, cost % 10);
    ...
    F: FnMut(&Point) -> bool + Copy,
{
    ...
    let is_cheaper = |(p, cost): &(Point, usize)| !point_costs.contains_key(p) || point_costs.get(p).unwrap() > cost;
